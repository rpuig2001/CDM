...
void CDM::OnFunctionCall(int FunctionId, const char* ItemString, POINT Pt, RECT Area) {
	...
	if (FunctionId == TAG_FUNC_EDITEOBT)
	{
		...
		{
			if (slotList[i].callsign == fp.GetCallsign()) {
				slotList[i].hasRestriction = false;
				slotList[i].ctot = "";
	...
	if (master && AtcMe) {
		...
		if (master && AtcMe) {
			...
			for (size_t i = 0; i < slotList.size(); i++) {
				if ((string)fp.GetCallsign() == slotList[i].callsign && slotList[i].hasRestriction && slotList[i].hasRestriction) {
							...
							if (stoi(calculatedTOBT) > stoi(GetTimeNow())) {
								...
								for (size_t i = 0; i < slotList.size(); i++)
								{
									if ((string)fp.GetCallsign() == slotList[i].callsign) {
										...
										if (slotList[i].hasRestriction && slotList[i].hasRestriction) {
											slotList[i].hasRestriction = false;
											slotList[i].ctot = "";
											slotList[i].hasRestriction = 0;
			...
			if (editedTOBT.length() == 4) {
				...
				if (hasNoNumber) {
					...
					for (size_t i = 0; i < slotList.size(); i++)
					{
						if (slotList[i].callsign == fp.GetCallsign()) {
							found = true;
							if (!slotList[i].hasRestriction || (slotList[i].hasRestriction && slotList[i].hasRestriction)) {
...
void CDM::OnGetTagItem(CFlightPlan FlightPlan, CRadarTarget RadarTarget, int ItemCode, int TagData, char sItemString[16], int* pColorCode, COLORREF* pRGB, double* pFontSize)
{
	if (ItemCode == TAG_ITEM_TSAT) {
		//Check master status
		if (myCallsign != ControllerMyself().GetCallsign() && ItemCode == TAG_ITEM_TSAT) {
			if (myCallsign == "") {
				myCallsign = ControllerMyself().GetCallsign();
			}
			else {
	...
	if (!isVfr) {
		...
		if (isCDMairport)
		{
			...
			bool hasCtot = false, hasManualCtot = false;
			if (aircraftFind) {
			...
			if (isValidToCalculateEventMode) {
				...
				if (master) {
					...
					if (stillOutOfTsat && !gndStatusSet) {
						...
						else if (ItemCode == TAG_ITEM_CTOT)
						{
							...
							if (aircraftFind) {
								...
								else if (slotList[pos].hasRestriction) {
									if (!inreaList) {
										ItemRGB = TAG_CTOT;
									}
									else {
										...
										if (hasCtot) {
											if (aircraftFind) {
												if (TTOTFinal != slotList[pos].ttot) {
													Plane p(callsign, EOBT, TSAT, TTOT, slotList[pos].ctot, slotList[pos].hasRestriction, slotList[pos].flowReason, hasManualCtot);
													slotList[pos] = p;
												...
												if (myCtot.empty()) {
													Plane p(callsign, EOBT, TSAT, TTOT, "", 0, "", hasManualCtot);
													for (size_t i = 0; i < slotList.size(); i++) {
													...
													Plane p(callsign, EOBT, TSAT, TTOT, myCtot, 0, "", hasManualCtot);
													slotList.push_back(p);
													pos = slotList.size() - 1;
												}
							...
							if (ItemCode == TAG_ITEM_TSAT) {
								...
								if ((remarks.find("CTOT") != string::npos && hasCtot) || (remarks.find("CTOT") != string::npos && hasManualCtot)) {
									...
									if (hasCtot) {
										...
										listCtot = slotList[pos].ttot.substr(0,4);
										if (listCtot != remarks.substr(remarks.find("CTOT") + 4, 4)) {
											if (remarks.find("%") != string::npos) {
												string stringToAdd = remarks.substr(0, remarks.find("CTOT")) + "CTOT" + listCtot + " " + remarks.substr(remarks.find("%"), remarks.length() - remarks.find("%"));
												FlightPlan.GetControllerAssignedData().SetFlightStripAnnotation(3, stringToAdd.c_str());
						...
						if (oldTSAT && aircraftFind) {
							if (slotList[pos].hasManualCtot || slotList[pos].hasRestriction) {
								oldTSAT = false;
							}
						}
						...
						else if (ItemCode == TAG_ITEM_E)
						{
							...
							if (aircraftFind) {
								if (slotList[pos].hasManualCtot) {
									...
									ItemRGB = TAG_YELLOW;
									strcpy_s(sItemString, 50, slotList[pos].flowReason.c_str());
								}
							}
						}
						else if (ItemCode == TAG_ITEM_CTOT)
					...
					if (aircraftFind) {
						if (!TSATFind && (!slotList[pos].hasRestriction || (slotList[pos].hasRestriction && slotList[pos].hasRestriction))) {
							slotList.erase(slotList.begin() + pos);
						}
						else if (TSATString != slotList[pos].tsat && TSATFind) {
							if (hasCtot) {
								Plane p(callsign, EOBT, TSATString, TTOTString, slotList[pos].ctot, slotList[pos].hasRestriction, slotList[pos].flowReason, hasManualCtot);
								slotList[pos] = p;
							}
							else {
								Plane p(callsign, EOBT, TSATString, TTOTString, slotList[pos].ctot, slotList[pos].hasRestriction, slotList[pos].flowReason, hasManualCtot);
						...
						if (TSATFind) {
							Plane p(callsign, EOBT, TSATString, TTOTString, "", 0, "", hasManualCtot);
							slotList.push_back(p);
						}
					}
					...
					if (TSATFind) {
						...
						if (remarks.find("CTOT") != string::npos) {
							...
							if (hasCtot) {
								if (slotList[pos].ctot != rmkCtot) {
									if (aircraftFind) {
										...
										slotList[pos].hasRestriction = true;
									}
								}
							}
							else {
							...
							if (aircraftFind) {
								slotList[pos].hasRestriction = false;
							}
						}
			...
			if ((string)fp.GetGroundState() != "STUP" && (string)fp.GetGroundState() != "ST-UP" && (string)fp.GetGroundState() != "PUSH" && (string)fp.GetGroundState() != "TAXI" && (string)fp.GetGroundState() != "DEPA") {
				...
				if (r.airport == dataRate.airport && r.arrRwyNo == dataRate.arrRwyNo && r.arrRwyYes == dataRate.arrRwyYes && r.dependentRwy == dataRate.dependentRwy
					...
					for (string dr : dataRate.depRwyYes) {
						...
						if (slotList[z].hasRestriction) {
							slotList[z].ctot = calculateTime(slotList[z].ctot + "00", (60.0 / stoi(r.rates[myPos]))).substr(0,4);
						}
					}
				}
			}
...
void CDM::backgroundProcess_recaulculate() {
	...
	for (size_t i = 0; i < slotList.size(); i++)
	{
		...
		if (!slotList[i].hasRestriction || inreaList) {
			...
			if (!aicraftInFinalTimesList) {
				...
				for (size_t s = 0; s < slotList.size(); s++)
				{
					if (myCallsign == slotList[s].callsign) {
						if (slotList[s].hasRestriction || slotList[s].hasManualCtot) {
							myhasCTOT = true;
						}
					}
				}
				...
				if (!myhasCTOT) {
					std::thread t(&CDM::refreshTimes, this, myFlightPlan, myCallsign, myEOBT, myTSAT, myTTOT, myAirport, myTTime, myRemarks, myDepRwy, dataRate, myhasCTOT, i, true);
					t.detach();
				...
				if (hasCTOT) {
					if (aircraftFind) {
						if (TTOTFinal != slotList[pos].ttot || slotList[pos].ttot == "999999") {
							Plane p(callsign, EOBT, TSAT, TTOT, myCTOT.substr(0,4), hasCTOT, myFlow, hasManualCtot);
							slotList[pos] = p;
							remarks = setCTOTremarks(remarks, p, FlightPlan);
						}
					}
					...
					if (aircraftFind) {
						if (TTOTFinal != slotList[pos].ttot) {
							Plane p(callsign, EOBT, TSAT, TTOT, "", false, myFlow, hasManualCtot);
							slotList[pos] = p;
							remarks = setCTOTremarks(remarks, p, FlightPlan);
						}
					}
		...
		if (!mySlotList[i].hasManualCtot) {
			...
			if ((string)myFp.GetGroundState() != "STUP" && (string)myFp.GetGroundState() != "ST-UP" && (string)myFp.GetGroundState() != "PUSH" && (string)myFp.GetGroundState() != "TAXI" && (string)myFp.GetGroundState() != "DEPA") {
				...
				if (ok) {
					...
					if (mySlotList[i].hasRestriction) {
						mySlotList[i].ctot = calculateTime(mySlotList[i].ctot + "00", timeToAdd);
					}
				}
			}
		}
					...
					for (Plane plane : slotList) {
						if (myfile.is_open())
						{
							if (airport == FlightPlanSelect(plane.callsign.c_str()).GetFlightPlanData().GetOrigin()) {
								...
								string str;
								if (plane.hasManualCtot) {
									str = plane.callsign + "," + plane.eobt + "," + plane.tsat + "," + plane.ttot + "," + plane.ttot.substr(0, 4) + ",MAN RESTRICTION" + ",";
								} else {
									if (plane.hasRestriction) {
										str = plane.callsign + "," + plane.eobt + "," + plane.tsat + "," + plane.ttot + "," + plane.ctot + "," + plane.flowReason + ",";
									}
									else {
...
void CDM::createJsonVDGS(vector<Plane> slotList, string fileName, string airport) {
	...
	for (Plane plane : slotList) {
		...
		} else if (plane.hasRestriction){
			Value ctot(plane.ctot.substr(0, 4).c_str(), document.GetAllocator());
			flight.AddMember("ctot", ctot, document.GetAllocator());
		}
...
void CDM::getCdmServerRestricted() {
	...
	if (responseCode == 404 || CURLE_OK != result) {
		...
		for (size_t i = 0; i < measures.size(); i++) {
			//Get callsign 
			string callsign = fastWriter.write(measures[i]["callsign"]);
			callsign.erase(std::remove(callsign.begin(), callsign.end(), '"'));
			callsign = callsign.substr(0, callsign.length() - 2);

			//Get CTOT
			string ctot = fastWriter.write(measures[i]["ctot"]);
			ctot.erase(std::remove(ctot.begin(), ctot.end(), '"'));
			ctot.erase(std::remove(ctot.begin(), ctot.end(), '\n'));
			ctot.erase(std::remove(ctot.begin(), ctot.end(), '\n'));

			//Get reason
			string reason = fastWriter.write(measures[i]["mostPenalizingAirspace"]);
...
void CDM::sendWaitingTSAT() {
	vector<string> tempList = setTSATlater;
	for (int i = 0; i < tempList.size(); i++) {
		for (Plane p : slotList) {
			sendMessage(p.callsign + "Is in the TSAT waiting list");
			if (p.callsign == tempList[i]) {
				setTSATlater.erase(setTSATlater.begin() + i);
				if (!p.hasRestriction) {
...
void CDM::setTSATApi(string myCallsign, string tsat) {
	...
	string taxiTime = getTaxiTime(myCallsign);
	...
	curl = curl_easy_init();
	if (curl) {
		curl_easy_setopt(curl, CURLOPT_URL, cdmServerUrl + "/slotService/cdm?callsign=" + myCallsign + "&taxi=" + taxiTime + "&tsat=" + tsat);
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WriteCallback);
		curl_easy_setopt(curl, CURLOPT_POST, 1L);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, &readBuffer);
		curl_easy_setopt(curl, CURLOPT_TIMEOUT, 20);
	...
	if (responseCode == 404 || CURLE_OK != result) {
		...
		if (obj.isMember("callsign")) {
			...
			for (size_t i = 0; i < tempList.size(); i++) {
				if (tempList[i].callsign == callsign) {
					...
					if (ctot != "") {
						...
						tempList[i].hasRestriction = false;
			...
			slotList = tempList;
			if (ctot != "") {
				CFlightPlan fp = FlightPlanSelect(myCallsign.c_str());
				addTimeToListForSpecificAirportAndRunway(10, calculateTime(GetTimeNow(), 5), fp.GetFlightPlanData().GetOrigin(), fp.GetFlightPlanData().GetDepartureRwy());
			}
		}
		else {
			sendMessage("Could not set TSAT " + tsat + " for " + myCallsign + ". CDM will automatically retry in 30 seconds...");
			setTSATlater.push_back(myCallsign);
		}
	}
}
...
string CDM::getTaxiTime(string callsign) {
	...
	for (size_t j = 0; j < taxiTimesList.size(); j++)
	{
		if (taxiTimesList[j].substr(0, taxiTimesList[j].find(",")) == myCallsign) {
