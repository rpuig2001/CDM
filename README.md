...
- vIFF Website: https://viffsys.com
...
### CDMconfig.xml
  ...
  - [OPTIONAL] Set only when issues with default provider - Set vIFF system URL. If empty or not set, default "https://viff-system.network" is used (Example: ```<viffSystem url="https://api.viffsys.com"```).
...
- ``.cdm startupdelay {airport}/{runway} {time_start}`` - Moves all TSATs for selected airport and runway from the starting at the time_start (time_start can be a 4 digits time (2114 - 21:14 time) or 1/2 digits minutes (5 - 5min or 10 - 10 min) - WAIT SOME SECONDS TO UPDATE AFTER APPLIED. After applying, all previous insterted delays for the same runway are removed. (Ex1. ``.cdm startupdelay LEBL/24L 1100`` -> All TSATs from LEBL rwy 24L will start at 1100 // Ex2. ``.cdm startupdelay LEBL/24L 10`` -> All TSATs will start at now+10 min). To remove the "restriction" use -> ".cdm startupdelay LEBL/24L 9999" (using 9999 as time).
- ``.cdm departuredelay {airport}/{runway} {time_start}`` - Moves all TTOTs for selected airport and runway from the starting at the time_start (time_start can be a 4 digits time (2114 - 21:14 time) or 1/2 digits minutes (5 - 5min or 10 - 10 min) - WAIT SOME SECONDS TO UPDATE AFTER APPLIED. After applying, all previous insterted delays for the same runway are removed. (Ex1. ``.cdm departuredelay LEBL/24L 1100`` -> All TTOTs from LEBL rwy 24L will start at 1100 // Ex2. ``.cdm departuredelay LEBL/24L 10`` -> All TTOTs will start at now+10 min). To remove the "restriction" use -> ".cdm departuredelay LEBL/24L 9999" (using 9999 as time).
